...
export default function AITable() {
  ...
  /* ───────── delete flow (toast‑safe) ───────── */
  const performDelete = async (id) => {
    const loadingId = toast.loading("Deleting…");
    try {
      await axios.delete(`http://localhost:5000/api/ai/${id}`);
      setAIList((prev) => prev.filter((ai) => ai._id !== id));

      toast.success("Deleted successfully!", {
        id: loadingId,   // replace spinner
        duration: 2500,  // auto‑hide
      });
    } catch {
    ...
    toast.custom(
      (t) => (
        <div className="bg-white shadow-lg rounded-lg p-4 flex items-center gap-4">
          ...
          <button
            onClick={() => {
              toast.dismiss(t.id);  // close confirm box
              performDelete(id);    // run delete flow
            }}
  ...
  /* ───────── toggle trending ───────── */
  const toggleTrending = async (id, cur) => {
    ...
    try {
      ...
      fetchAI();
      toast.success("Updated", { id: tId, duration: 2500 });
    } catch {
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <h2 className="text-xl font-semibold text-[#2C4964]">AI Tools</h2>
